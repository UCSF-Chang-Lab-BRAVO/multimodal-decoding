...
class Seq2seqBaseline(nn.Module):
    def __init__(self, rnn_dim, KS, num_layers, dropout, n_targ_ctc, n_targ_s2s, bidirectional=True, in_channels=506, 
        ...
        self.ctc_dense = nn.Linear(rnn_dim, n_targ_ctc)
    ...
    def encode(self, x, lens):
        ...
        return unpacked_output, encoder_mask, final_hidden
    ...
    def compute_loss(self, source, lens, target, device):
        ...
        # suffix A B C </s>.s
        encoder_output, encoder_mask, encoder_hidden = self.encode(source, lens)
#         lengths = torch.sum(source != pad_id, axis=0)
        ...
        loss = torch.nn.functional.cross_entropy(logits, decoder_targs.to(device), ignore_index=0, label_smoothing=self.smoothing)
        return loss
    
class Seq2seqAttention(Seq2seqBaseline):
    def __init__(self, rnn_dim, KS, num_layers, dropout, n_targ_ctc, n_targ_s2s, bidirectional=True, in_channels=506, smoothing=0):
        super().__init__(rnn_dim, KS, num_layers, dropout, n_targ_ctc, n_targ_s2s, bidirectional=True, in_channels=in_channels, smoothing=smoothing)
