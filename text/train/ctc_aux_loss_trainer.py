...
def train_f(model, train_loader, optimizer, device, wordct_weight=0.1, clipamt=1.0): 
    """
    Train a ctc model. 
    """
    total_loss = 0
    ...
    for x, y, l, targ_len, _, wordct in train_loader: 
        x = x.float().to(device)
        y = y.long().to(device)
        l = l.int().cpu()
...
#         print(emissions.shape, y.shape)
        ...
        loss += wordct_weight*class_loss(word_ct_pred, wordct)
...
def test_f(model, test_loader, optimizer, device, greedy, beam_search_decoder, texts, tokens, print_greedy=False, verbose=True, printall=False,
          print_hypo=False, wordct_weight=0.0):
    """
    ...
    with torch.no_grad():
        ...
        total_per = 0
        gts, transcripts = [], []
        for x, y, l, targ_len, gtsent, wordct in test_loader: 
            x = x.float().to(device)
            y = y.long().to(device)
            l = l.int().to(device)
            wordct = wordct.long().to(device)
            ...
            emissions, word_ct_hat, lengths = model(x, l)
            
            emissions = F.log_softmax(emissions, dim=-1)
            loss = loss_fn(emissions, y, lengths, targ_len)
            total_loss += loss.item()
            total_loss += wordct_weight*class_loss(word_ct_hat, wordct)
            total_samps += x.shape[0]
...
def train_loop(model, train_loader, test_loader, optimizer, device, 
              ...
              print_hypo=False, wordloss_weight=0.1, 
              clipamt=1.0):
    """
    ...
    for epoch in range(max_epochs):
        if train:
            tr_loss, model = train_f(model, train_loader, optimizer, device, wordloss_weight, clipamt= clipamt)
        else: 
            tr_loss = np.nan
        te_loss, model, wer, cer, per = test_f(model, test_loader, optimizer, 
                                               ...
                                               print_hypo=print_hypo, 
                                              wordct_weight=wordloss_weight)
        ...
        if not wer is None: 
            ...
            if wer < best_wer: 
                ...
                if not checkpoint_dir is None:
                    if wer < .85:
                        import os
                        torch.save(model.state_dict(), os.path.join(checkpoint_dir, str(wandb.run.name) + f"_{epoch}.pth"))
            else:
                patience_ctr +=1
